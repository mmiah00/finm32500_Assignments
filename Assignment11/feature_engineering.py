...
market_data_df['SMA'] = market_data_df.groupby('ticker')['close'].pct_change().expanding().mean()
market_data_df['RSI'] = market_data_df.groupby('ticker')['close'].transform(lambda prices, period=20: 100 - (100 / (1 + prices.diff().clip(lower=0).ewm(com=period-1, adjust=False).mean() / -prices.diff().clip(upper=0).ewm(com=period-1, adjust=False).mean())))
...
print(market_data_df['Label'].describe())
