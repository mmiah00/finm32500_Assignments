...
class ModelRunner:
    def __init__(self, df):
        self.folder = './Assignment11/'
        featuresjson = 'features_config.json'
        modeljson = 'model_params.json'
        self.features = json.load(open(self.folder + featuresjson, 'r'))
        self.model_params = json.load(open(self.folder + modeljson, 'r'))
        self.market_data_df = df
        self.X = self.market_data_df[['close']].values
        self.y = self.market_data_df[['Label']].values.ravel()
